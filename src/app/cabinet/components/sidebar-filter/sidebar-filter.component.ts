...
export class SidebarFilterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subscription$.add(
      combineLatest(selectFilter$, routeFilter$)
        .subscribe(([selectFilter, routeFilter]) => {
          if (selectFilter) {
            this.filterModel = selectFilter;
          } else if (routeFilter) {
            this.filterModel = this.filterStoreService.getNewFilterByParams(routeFilter);
          } else {
  ...
  onSubmit() {
    this.router.navigate([], {
      ...
      queryParams: this.filterStoreService.getFilterByParamsForQueryParams(this.filterModel)
    });
  }
