...
export class CabinetComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.isShowSidebarFilter$ = store.pipe(
      select(fromRoot.getShowSidebarFilter)
    );
    ...
    this.subscription$.add(
      store.pipe(select(fromRoot.getProjectSelectedId)).subscribe((id) => {
    ...
    this.subscription$.add(
      route.params
        .pipe(
          map((params) => params.projectId), // Используйте map вместо pluck
