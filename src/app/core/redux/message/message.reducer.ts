...
export const adapter: EntityAdapter<MessageModel> = createEntityAdapter<MessageModel>({
...
export const getEntitiesArray = createSelector(getEntities, getIds, (entities, ids) => ids.map((id: string) => entities[id]));
export const getEntityEdit = createSelector(getEntities, getEditId, (entities, id) => entities[id]);
export const getEntitiesArrayByTaskId = (taskId: string) => createSelector(getEntitiesArray, (entities: MessageModel[]) => {
  return entities.filter((item: MessageModel) => item.taskId === taskId);
});
export const getEntitiesArrayPinned = createSelector(getEntitiesArray, (entities: MessageModel[]) => {
