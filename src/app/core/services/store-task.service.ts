...
export class StoreTaskService {
  ...
  private _prepareAll(taskList$: Observable<TaskModel[]>) {
    ...
    return combineLatest(taskList$, inviteList$, statusList$, tagList$)
      .pipe(
        filter(([taskList, inviteList, statusList, tagList]) => inviteList.length > 0 && statusList.length > 0),
        map(([taskList, inviteList, statusList, tagList]) => {
          if (taskList && inviteList && statusList && tagList) {
            taskList.forEach((task: TaskModel) => {
              if (task.responsibleId) {
                task.responsible = _.find(inviteList, {'userId': task.responsibleId as any});
              }
  ...
  private _prepareOne(task$: Observable<TaskModel>) {
    ...
    return combineLatest(task$, inviteList$, statusList$, tagList$)
      .pipe(
        filter(([task, inviteList, statusList, tagList]) => task && inviteList.length > 0 && statusList.length > 0),
        map(([task, inviteList, statusList, tagList]) => {
          ...
          if (task.userIdArchive) {
            task.userArchive = _.find(inviteList, {userId: task.userIdArchive as any});
          }

          if (task.userIdComplete) {
            task.userComplete = _.find(inviteList, {userId: task.userIdComplete as any});
          }
